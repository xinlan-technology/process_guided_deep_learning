...
class DataConfig:
    ...
    def __init__(self):
        # Weather features for prediction (only 7 weather variables for deep learning)
...
class DataProcessor:
    ...
    def preprocess_data(self, input_data, temp_data, ice_data, is_simulation=False):
        ...
        if is_simulation:
            lake_data = lake_data[['Date', 'ShortWave', 'LongWave', 'AirTemp', 'RelHum',
                              'WindSpeed', 'Rain', 'Snow', 'depth', 'wtemp']]
        else:
            lake_data = lake_data[['Date', 'ShortWave', 'LongWave', 'AirTemp', 'RelHum',
    ...
    def prepare_lake_data(self, use_simulation=False):
        ...
        # Only weather features for deep learning (7 features)
        weather_day = lake_data.groupby('Date')[self.config.weather_features].mean()
        
        if not use_simulation:
            # Only extract daynum if present
            daynum_day = lake_data.groupby('Date')['daynum'].first()
        ...
        # Only add daynum if not using simulation
        if not use_simulation:
            daily_data = pd.merge(
                daily_data,
                daynum_day.reset_index(name='daynum'),
        on='Date'
        )

        # Copy daily data for scaling
        daily_data_scaled = daily_data.copy()

        # Scale only the 7 weather features (for deep learning)
